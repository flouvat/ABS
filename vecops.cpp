...
{                               /* --- heap sort for pointer vectors */
  ...
  while (--l >= 0)              /* while the heap is not complete, */
    _sift( (void**)vec, l, r, cmpfn, data);     /* extend it by one element */
  v = (void**)vec;                      /* type the vector pointer */
...
{                               /* --- shuffle vector entries */
  int  i;                       /* vector index */
  void **v = (void**)vec, *t;           /* vector and exchange buffer */
...
{                               /* --- reverse a pointer vector */
  ...
  for (v = (void**) vec; --n > 0; ) {    /* reverse the order of the elements */
    t = v[n]; v[n] = v[0]; *v++ = t; }
}  /* v_reverse() */
