...
static int _getsupp (ISNODE *node, int *sets, int cnt)
{                               /* --- get support of an item set */
  int    i, c;                  /* vector index, number of children */
  ISNODE **vec;                 /* vector of child nodes */

  assert(node && sets && (cnt >= 0)); /* check the function arguments */
  while (--cnt > 0) {           /* follow the set/path from the node */
    ...
    vec = (ISNODE**)(node->cnts +node->size);
    i   = *sets++ -vec[0]->id;   /* compute the child vector index */
  ...
  i = *sets -node->offset;       /* compute the counter index */
  if ((i < 0) || (i >= node->size))
...
static void _clrsupp (ISNODE *node, int *sets, int cnt, int supp)
...
void ist_countx (ISTREE *ist, TATREE *tat, int & nbfreq, int minsup)
{                               /* --- count transaction in tree */
  assert(ist && tat);           /* check the function arguments */
  _countx(ist->levels[0], tat, ist->lvlcnt, nbfreq, minsup );
...
void ist_filter2 (ISTREE *ist, int mode,Trie * bdn)
{                               /* --- filter max. freq. item sets */
  int    i, k, n;               /* loop variables */
  ISNODE *node, *curr;          /* to traverse the nodes */
  int    supp;                  /* support of an item set */
  ...
  for (n = 1; n < ist->lvlcnt; n++) {
    for (node = ist->levels[n]; node; node = node->succ) {
	  ...
	  short unsigned * itemset = new short unsigned[ n+1 ] ; 

	  int  j = n+1;
	  itemset[ --j ] = node2->offset +i ;
	  ...
	  if ( bdn != 0 )
	  	   bdn->insert( itemset, n+1, node->cnts[i] ) ;
...
int ist_set (ISTREE *ist, int *sets, int *supp)
{                               /* --- extract next frequent item set */
  int    i;                     /* loop variable */
  ISNODE *node;                 /* current item set node */

  assert(ist && sets && supp);   /* check the function arguments */
  ...
  *supp    = i;                 /* note the set support */
  ...
  sets[--i] = node->offset +ist->index;       /* store the first item */
  while (node->parent) {        /* while not at the root node */
    sets[--i] = node->id;        /* add item to the item set */
